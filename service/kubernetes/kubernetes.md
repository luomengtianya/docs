...
![image-20221104205423941](kubernetes.assets/image-20221104205423941.png)
...
* 修改kubernetes-dashboard deployment,启用新的secret
  ...
  ![image-20211125114547930](kubernetes.assets/image-20211125114547930.png)

​		因为这不是被信赖的机构颁发的证书，但是这只是我们自己使用的话，可以忽略这个问题；若是需要对外提供服务，最好还是使用被信赖的证书。

![image-20221104205718112](kubernetes.assets/image-20221104205718112.png)

​	直接点击证书拖动到电脑界面，证书就会被复制出来，然后双击证书就可以安装

![image-20221104205732174](kubernetes.assets/image-20221104205732174.png)

​	证书被信任之后，就可以继续访问了

![image-20221104205747017](kubernetes.assets/image-20221104205747017.png)
...
* 登陆网站并设置token数据
  ...
  ![image-20221104205806169](kubernetes.assets/image-20221104205806169.png)
...
![image-20221104205831395](kubernetes.assets/image-20221104205831395.png)
...
![image-20221104205845968](kubernetes.assets/image-20221104205845968.png)
...
![image-20221104205901097](kubernetes.assets/image-20221104205901097.png)



一个扇出（fanout）配置根据请求的 HTTP URI 将来自同一 IP 地址的流量路由到多个 Service。 Ingress 允许你将负载均衡器的数量降至最低

![image-20221104205911161](kubernetes.assets/image-20221104205911161.png)
...
nohup kubectl port-forward --namespace=ingress-nginx service/ingress-nginx-controller 443:443 --address='0.0.0.0' &
```
